...
function getTwoOrFour() {
    ...
    if (randomChance <= 0.1) return 4;
    return 2;
}
function filledCellsCount(board) {
    let count = 0;
    for (const row of board){
        for (const cell of row)if (cell !== 0) count++;
    }
    return count;
}
function setTwoOrFour(board) {
    ...
    while(true){
        for(let row = 0; row < board.length; row++)for(let cell = 0; cell < board[row].length; cell++){
...
function compareFields(startField, newField) {
    for(let r = 0; r < newField.length; r++)for(let c = 0; c < newField[r].length; c++){
        if (startField[r][c] === 2048) return "win";
    }
    for(let r = 0; r < newField.length; r++)for(let c = 0; c < newField[r].length; c++){
        if (startField[r][c] !== newField[r][c]) return false;
    }
    return true;
}
function addNewTwoOrFour(board, startState) {
    if (compareFields(startState, board) === false) setTwoOrFour(board);
...
function moveLeftHelper(board, boardScore) {
    ...
    const startBoardState = board.map((row)=>[
            ...row
        ]);
    ...
    for (const row of boardCopy){
        ...
        for(let i = 0; i < filtered.length; i++){
            ...
            } else if (filtered[i + 1] === 0 && filtered[i] !== 0) filtered[i + 1] = filtered[i];
        }
        filtered = filtered.filter((x)=>x !== 0);
        for(let i = 0; i <= 3; i++)if (!filtered[i]) filtered.push(0);
...
function moveDownHelper(board, boardScore) {
    ...
    for(let c = 0; c < boardCopy[0].length; c++){
        const startRowState = [];
        for(let r = 0; r < boardCopy.length; r++)startRowState.push(boardCopy[r][c]);
        ...
        for(let cell = 0; cell < filtered.length; cell++){
            ...
            } else if (filtered[cell + 1] === 0 && filtered[cell] !== 0) filtered[cell + 1] = filtered[cell];
        }
        filtered = filtered.filter((x)=>x !== 0);
        for(let i = 0; i <= 3; i++){
            if (filtered.length < 4) filtered.unshift(0);
            else break;
        }
        for(let row = 0; row < boardCopy.length; row++)boardCopy[row][c] = filtered[row];
    }
    const comparingStartToCurrBoard = compareFields(startBoardState, boardCopy);
    return [
        boardCopy,
...
class Game {
    ...
    getStatus() {
        ...
        const winMessage = document.querySelector(".message-win");
        if (!loseMessage.classList.contains("hidden")) return "lose";
        else if (!winMessage.classList.contains("hidden")) return "win";
        else if (startMessage.classList.contains("hidden")) return "playing";
        else return "start";
    }
    moveLeft() {
        document.addEventListener("keydown", (ev)=>{
            if (ev.key === "ArrowLeft") {
                ...
                const loseMessage = document.querySelector(".message-lose");
                if (mLeftRes === true && mRightRes === true && mDownRes === true && mUpRes === true) loseMessage.classList.remove("hidden");
    ...
    restart() {
        ...
        if (restartButton && !restartButton.classList.contains("modifyied")) restartButton.addEventListener("click", (ev)=>{
